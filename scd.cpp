...
void get_subsq(int subs, const std::vector<int>& in, std::vector<int>& out){
    // write your code here
    int side = std::sqrt(in.size()), subSide = std::sqrt(side);
    int rowStart = subSide*(div(subs, subSide).quot);
    int colStart = subSide*(div(subs, subSide).rem);

    for(int i = rowStart; i < rowStart + subSide; i++){
        for(int j = colStart; j < colStart + subSide; j++){
